...
pub fn encode_sample(
    ...
    let mut dest = match preset {
        Some(p) => input.with_extension(format!("x265.crf{crf}.{p}.{dest_ext}")),
        None => input.with_extension(format!("x265.crf{crf}.{dest_ext}")),
    };
    ...
    let enc = Command::new("ffmpeg")
        ...
        .arg2("-c:v", "libx265")
        .arg2("-crf", crf)
        .arg2_opt("-x265-params", keyint.map(|i| format!("keyint={i}")))
        ...
        .stderr(Stdio::piped())
        .spawn()
        .context("ffmpeg libx265")?;

    let stream = FfmpegOut::stream(enc, "ffmpeg libx265");
    Ok((dest, stream))
}
...
pub fn encode(
    ...
    Ok(FfmpegOut::stream(enc, "ffmpeg libx265"))
}
